...
<xsl:stylesheet xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template match="*" mode="metadata">
    ...
    <!-- Management Info VRE -->
    <!-- <xsl:for-each select="gmd:vre/gmd:LW_VRE/gmd:managementInfo_vre/gmd:LW_ManagementInfo/gmd:author_vre[normalize-space(gco:CharacterString) != '']">
        <xsl:variable name="author_vre_variable">
            <xsl:for-each select="gco:CharacterString">
                <xsl:value-of select="concat(., '')" /> inserire doppio trattino tra apici
            </xsl:for-each>
        </xsl:variable>	   
        <Field name="author_vre"
	                 string="{string($author_vre_variable)}" 
	                 store="true" index="false"/>
	</xsl:for-each> -->
